...
- **在FLUX.1 Dev与Stable Diffusion 3.5 Large上，SafeDIG可稳定降低目标域不安全生成率，同时保留原安全性与生成质量** ⭐
  ...
  - Idea hook：可将该位置感知稀疏特征迁移思路扩展到大模型安全对齐，探索跨风险域安全适配新方案
...
- **BRACS可降低CHAIR$_s$ 9.4个点、提升POPE F1 2.7个点，平均推理速度比基线高1.3倍。** ⭐
  ...
  - Idea hook：这种按需自适应干预的引导思路，可迁移到通用大语言模型的事实幻觉缓解研究中。
...
- **对Qwen-2.5模型族分析发现，性能更优的数学推理具备更少步骤、单步信息密度更高的特性**
  ...
  - Idea hook：可将该推理模式引导思路用于优化知识编辑后小模型，提升其推理性能与可靠性
...
- **仅更新模型0.11%的总参数，在少样本与领域偏移设置下性能始终优于现有最优方法。** ⭐ 💻
  ...
  - Idea hook：可将证据不确定性适配思路拓展到大模型知识编辑，提升编辑后模型的域泛化鲁棒性。
...
- **结合结果正确性与对数几率引导校准教师对数，可稳定自蒸馏，性能优于标准同策略自蒸馏**
  ...
  - Idea hook：可将该结果引导校准思路引入知识编辑后输出校准，缓解编辑偏差，提升编辑可靠性
...
- **分阶段的条件引导与混合专家合成设计，让FineSteer在极小效用损失下获得优于SOTA的引导效果** ⭐
  ...
  - Idea hook：可将该细粒度推理引导思路适配到知识编辑场景，缓解编辑后模型通用能力下降的问题
...
- **均值差注入在14个LLM中11个优于基线，增益3.6%-16.4%，混合方法在13个模型上性能更优**
  ...
  - Idea hook：可将语义校准无界搜索思路迁移到知识编辑的干预优化中，提升知识编辑效果
...
- **通过多视角分析证实，大语言模型人格存在情境依赖性，且具有稳定一致的情境-行为关联模式**
  ...
  - Idea hook：可将这种神经元级情境引导思路迁移到知识编辑，提升编辑结果的情境适配能力
...
- **稀疏解耦表征的相位引导效果优于稠密或纠缠表征，静态干预无法适应该动态场景**
  ...
  - Idea hook：可借鉴稀疏自动编码器提取解耦可解释特征作为可控编辑轴，拓展知识编辑应用场景
...
- **在去噪早期修正计数偏差可避免结构错误固化，动态调整比静态引导以少量计算换得更高计数准确率，高目标计数提升显著。**
  ...
  - Idea hook：可将测试时间自适应引导思路引入知识编辑领域，优化生成约束满足度，提升知识编辑输出可靠性。
...
- **VLM的安全决策高度依赖语义线索，依赖习得图文关联而非接地视觉理解，易被自动引导流程利用** ⭐
  ...
  - Idea hook：可研究如何修正多模态安全模型的偏置，使其依赖接地视觉理解而非浅层语义提升鲁棒性
...
- **基于多特质子空间引导生成的黑暗模型，在单轮和多轮评估中均能稳定产生有害交互与有害结果**
  ...
  - Idea hook：可借鉴该子空间引导思路，调控大模型有害行为，开展人机交互安全与可靠性研究
...
- **为编辑区域分配更高噪声、未编辑区域分配更低噪声，可同时提升编辑效果与内容一致性**
  ...
  - Idea hook：可将分区域差异化噪声引导的思路引入知识编辑，优化编辑后原有正确知识的保留效果
...
- **仅960M参数的EditMGT在多个基准取得最优图像相似性，编辑速度比扩散方法快6倍**
  ...
  - Idea hook：可借鉴该工作约束编辑范围的思路，解决知识编辑中修改串扰非目标知识的问题
...
- **设计合理的编辑范式下，顺序分解随任务复杂度提升仍能带来鲁棒性能提升，合成学到的分解能力可迁移到真实图像**
  ...
  - Idea hook：可借鉴本文顺序分解加合成数据训练的思路，解决多步复杂知识编辑的误差累积问题
...
- **相比先前时间一致性最优基线，词错率降低近70%，在基础TTS上WDTW降低27%，取得SOTA性能。**
  ...
  - Idea hook：可借鉴该无训练编辑、边界自适应约束思路，解决知识编辑中保留原知识一致性的问题。
...
- **采用三层分工的分层多智能体框架，在视频混剪的客观指标和人工评估中均显著优于现有最优方法** 💻
  ...
  - Idea hook：可借鉴该分层多智能体分工协调思路，解决复杂多模态场景下的知识编辑任务
...
- **AEvo相对最强基线取得26%的相对提升，相同迭代预算下开放优化任务达到当前最优性能** ⭐
  ...
  - Idea hook：可将元智能体编辑进化机制的思路引入知识编辑，解决长周期知识编辑的漂移问题
...
- **恶意知识编辑可稳定诱导错误不安全推理，同时基本保留模型通用能力，这类风险难以检测，受三类关键因素影响** ⭐
  ...
  - Idea hook：可基于EditRisk-Bench开发抵御恶意知识注入的安全编辑方法，降低知识编辑引入的推理安全风险
...
- **经3000次顺序编辑后，SCAN仍可在MMLU、GSM8K保持模型完整性，现有方法会逐渐恶化最终发生模型崩溃** ⭐
  ...
  - Idea hook：可扩展探索稀疏电路锚定机制在大模型海量编辑场景下的稳定性与泛化能力
