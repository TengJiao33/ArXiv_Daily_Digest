...
def backfill_file(
    ...
    if selected and write:
        annotate_venues(selected, use_semantic_scholar=semantic_scholar, delay=delay)
...
def parse_args():
    ...
    parser.add_argument("--manual-only", action="store_true", help="Use only venue_overrides.yaml, no Semantic Scholar calls.")
...
def main():
    ...
    print(f"[VenueBackfill] Semantic Scholar lookup={'off' if args.manual_only else 'on'}")
