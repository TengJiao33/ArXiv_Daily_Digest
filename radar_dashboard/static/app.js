...
function renderSummary() {
  ...
  $("subtitle").textContent = `本地数据视图 · ${summary.current_week || "no week"}`;
  ...
  setOptions($("weekSelect"), weekOptions, state.week || summary.current_week);
...
function renderDirections(summary) {
  $("directionCards").innerHTML = summary.directions.map((direction, index) => {
    const themes = direction.themes.slice(0, 4).map(([theme, count]) => (
      `<span class="tag">${escapeHtml(theme)} ${count}</span>`
    )).join("");
    return `
      <article class="direction-card" style="border-left:4px solid ${colors[index % colors.length]}">
        ...
        <div class="paper-meta">${themes}</div>
      </article>
    `;
  }).join("");
}
...
function renderThemes(summary) {
  const chips = summary.themes.map(([theme, count]) => (
    `<button class="chip ${state.theme === theme ? "active" : ""}" type="button" data-theme="${escapeHtml(theme)}">${escapeHtml(theme)} · ${count}</button>`
  )).join("");
  $("themeCloud").innerHTML = `<button class="chip ${state.theme === "all" ? "active" : ""}" type="button" data-theme="all">全部</button>${chips}`;
...
function renderLimitations(summary) {
  $("limitationsList").innerHTML = summary.limitations.map(([text, count]) => (
    `<li>${escapeHtml(text)} <span class="tag">${count}</span></li>`
  )).join("") || `<li>本周摘要未显式提到 limitation</li>`;
}
  ...
  return `
    <article class="paper-card">
      ...
      <div class="paper-meta">
        ...
        <span class="tag">${escapeHtml(paper.theme)}</span>
        ${paper.has_code ? `<span class="tag code">代码</span>` : ""}
      </div>
      <dl>
        <div><dt>Method</dt><dd>${escapeHtml(paper.method || "未提取")}</dd></div>
        <div><dt>Contribution</dt><dd>${escapeHtml(paper.contribution || "未提取")}</dd></div>
        <div><dt>Key Finding</dt><dd>${escapeHtml(paper.key_finding || "未提取")}</dd></div>
        <div><dt>Limitation</dt><dd>${escapeHtml(paper.limitations || "摘要未提及")}</dd></div>
      </dl>
...
async function loadPapers() {
  ...
  $("papers").innerHTML = data.papers.map(paperCard).join("") || `<p class="empty">没有匹配论文</p>`;
}

async function loadAll() {
  state.summary = await fetchJson("/api/summary");
  if (!state.week) state.week = state.summary.current_week;
  renderSummary();
  await loadPapers();
}
